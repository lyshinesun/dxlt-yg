        ...
        get_in_stock_list: function () {
            ...
            var Parameters = {
                "parameters": {
                    ...
                    "fd_page_index": "1",
                    "fd_page_size": "10",
                    "fd_order_class": this.in_stock_type,//入库类型
                    "fd_code": "",//入库编号（非必填)
                ...
                if ($("#stock_form", iframeO).valid()) {
                    ...
                    var Parameters = {
                        "parameters": {
                            ...
                            "fd_order_class": $('#device_type option:selected', iframeO).val(),
        ...
        popDevice: function (e) {
            ...
            if (obj.attr('id') == 'create_device') { //新增库存
                ...
                parent.layer.open({
                    ...
                    area: ['800px', '620px'],
                    content: ['popUp/pop_in_stock.html'],
                    success: function (layero, index) {
                        ...
                        _this.addChangeClick(iframeO, '新建入库单成功');
                ...
                if (obj.attr('id') == 'device_delete') { //删除入库单
                    var Parameters = {
                        "parameters": {
                            "fd_ids": fdId,
                            "stationid": this.stationId
                ...
                } else if (obj.attr('id') == 'device_modify') { //修改入库单
                    _this.getAllStation();
                    var Parameters = {
                        "parameters": {
                            "fd_id": fdId,
                        ...
                        if (res.success) {
                            parent.layer.open({
                                ...
                                success: function (layero, index) {
                                    ...
                                    $('#fdWstartDatetime', iframeO).val(res.data.fd_log_date);
                                    $('#workorder_code', iframeO).val(res.data.fd_work_code); //工单号
                                    $('#task_msg', iframeO).val(res.data.fd_order_class); //入库单类型
                                    $('#fd_desc', iframeO).val(res.data.fd_desc); //入库单描述


                                    $("#stock_form", iframeO).validate();

                                    _this.addChangeClick(iframeO, '修改入库单成功');
