            ...
            vlm.loadJson("", JSON.stringify(Parameters), function (res) {
                if (res.data) {
                    ...
                    if(unitArr.length){
                        ...
                        $('#psBlock .nbq_td10').off().click(function(){
                            var devid=$(this).find('input[name="batchIds"]').attr('data-devid');
                            var fd_name=$(this).find('input[name="batchIds"]').val();
                            parent.layer.open({
                                type: 2,
                                title: '组串逆变器支路信息 逆变器:'+fd_name,
                                shadeClose: true,
                                shade: 0.5,
                                maxmin: true, //开启最大化最小化按钮
                                area: ['1000px', '600px'],
                                content: ['popUp/inverter_pop.html'],
            ...
            vlm.loadJson("", JSON.stringify(Parameters), function (res) {
                ...
                if(_this.pointId_cap == "ac_pw"){  //普通交流功率
                    for (var i = 0; i < data.length; i++) {
                        ...
                        powerDates.push(data[i].fd_datetime.substring(11, 16));
                    }
                }else if(_this.pointId_cap == "ac_pw_one"){
                    //归一化
                    for (var i = 0; i < data.length; i++) {
