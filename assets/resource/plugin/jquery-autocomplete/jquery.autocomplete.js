 ...
 *  modify by ghouman add enableCookie ，enableCookie属性设置为true的时候 绑定文本框的name设置cookie值，组件将会根据文本框的name
 *  取值并且在鼠标定位的时候显示 cookie的值
 */
...
(function (factory) {
    ...
    function Autocomplete(el, options) {
        var noop = function () { },
            ...
            defaults = {
                ...
                enableCookie:false //add by ghouman (element will show value from cookie when set enableCookie is true)
            };
    ...
    Autocomplete.prototype = {
        ...
        //未输入值的时候获取记忆的cookie值 add by ghouman
        getCookieValue: function (){
            var s = getCookie(this.element.name);
            var array = [];
            if(s && s.length>0){
                var a = s.split(",");
                var obj = {};
                $.each(a, function(i) {
                    array.push({value: a[i]+'', data: a[i]});
                });
            }
        ...
        onFocus: function (){
            //当用户为输入值的时候，自动弹出记忆的内容 add by ghouman
        ...
        getSuggestions: function (q) {
            ...
            if(options.enableCookie&&!q){//enableCookie support
                that.suggestions = that.getCookieValue();
                that.suggest();
                return;
            }
        ...
        isBadQuery: function (q) {
            if(q==undefined){//add magm 20170122
                return false;
            }
        ...
        adjustScroll: function (index) {
            ...
            //改成返回data modify by ghouman
            that.el.val(that.suggestions[index].data);
        },
